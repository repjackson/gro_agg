if Meteor.isClient
    @selected_user_videos = new ReactiveArray []
    ...
    Template.videos.onCreated ->
        ...
        @autorun -> Meteor.subscribe 'selected_videos', 
            selected_user_videos.array() 
    ...
    Template.videos.helpers
        videos: ->
            ...
            if selected_user_videos.array().length > 0 then match.videos = $all: selected_user_videos.array()
            Docs.find match,
                sort:points:-1
            # if Meteor.user()
    ...
    #     'click .debit': ->
    #         # user = Meteor.videos.findOne(username:@username)
    #         new_debit_id =
    #             Docs.insert
    #                 model:'debit'
    #                 recipient_id: @_id
    ...
    #         Meteor.videos.update Template.parentData(1)._id,
    #             $addToSet:
    #                 "#{@key}": @value
    ...
    Template.image_cloud.onCreated ->
        @autorun -> Meteor.subscribe('videos',
            selected_user_videos.array()
            Session.get('selected_user_site')
            Session.get('selected_user_location')
    ...
    Template.image_cloud.helpers
        all_videos: -> results.find(model:'user_image')
        ...
        selected_user_videos: -> selected_user_videos.array()
        selected_user_location: -> Session.get('selected_user_location')
        # all_site: ->
        #     user_count = Meteor.videos.find(_id:$ne:Meteor.userId()).count()
    ...
    Template.image_cloud.events
        'click .select_image': -> 
            ...
            selected_user_videos.push @name
        'click .unselect_image': -> selected_user_videos.remove @valueOf()
        'click #clear_videos': -> selected_user_videos.clear()
...
if Meteor.isServer
    Meteor.publish 'selected_videos', (
        selected_user_videos
        selected_user_site
        ...
        if selected_user_videos.length > 0 then match.videos = $all: selected_user_videos
